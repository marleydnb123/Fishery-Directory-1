...
const AccommodationPage: React.FC = () => {
  ...
  // Fetch accommodation + fisheries info from Supabase
  useEffect(() => {
    const fetchAccommodations = async () => {
      ...
      if (error) {
        ...
        // Only keep accommodations that have a linked fishery
        setAccommodations(data.filter(acc => acc.fisheries));
      }
      setLoading(false);
    };
    fetchAccommodations();
  ...
  // Build unique districts and species from fetched data
  const districts = Array.from(
    new Set(accommodations.map(acc => acc.fisheries?.district).filter(Boolean))
  );
  const species = Array.from(
    new Set(
      accommodations
        .flatMap(acc => Array.isArray(acc.fisheries?.species) ? acc.fisheries.species : [])
        .filter(Boolean)
    )
  );
  ...
  useEffect(() => {
    ...
    if (selectedDistrict) {
      results = results.filter(acc => acc.fisheries?.district === selectedDistrict);
    }
    if (selectedSpecies) {
      results = results.filter(acc =>
        Array.isArray(acc.fisheries?.species) && acc.fisheries.species.includes(selectedSpecies)
      );
    }
            ...
            {filteredAccommodations.map(accommodation => (
              <motion.div key={accommodation.id} variants={itemVariants}>
                <AccommodationCard
                  accommodation={{
                    ...accommodation,
                    fishery: accommodation.fisheries // pass fishery info as "fishery" prop
                  }}
                />
              </motion.div>
            ))}

...
const AdminFisheries: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'features') {
      // Allow spaces in input, only split on commas
      const features = value.split(',').map(s => s.trim()).filter(Boolean);
      setFormData(prev => ({ ...prev, features }));
    } else if (name === 'facilities') {
      // Allow spaces in input, only split on commas
      const facilities = value.split(',').map(s => s.trim()).filter(Boolean);
      setFormData(prev => ({ ...prev, facilities }));
    } else if (name === 'fishing_type') {
      // Allow spaces in input, only split on commas
      const fishingTypes = value.split(',').map(s => s.trim()).filter(Boolean);
      setFormData(prev => ({ ...prev, fishing_type: fishingTypes }));
    } else if (name === 'species') {
      // Allow spaces in input, only split on commas
      const species = value.split(',').map(s => s.trim()).filter(Boolean);
